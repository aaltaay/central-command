const fs = require('fs');

async function submit() {
  const cookie = 'PrestaShop-7a627346ffd9a0d2a487b9c507e59655=';

  const formData = new FormData();
  formData.append('submit_send_ticket', '2');
  formData.append('fields[5]', 'Ahmed Altaay');
  formData.append('fields[6]', 'aaltaay2@gmail.com');
  formData.append('fields[7]', 'Where is my order? Why is it not fulfilled yet?');
  formData.append('fields[11]', 'PYRLTKUDO');
  formData.append('fields[10]', 'Please check on this or give me my money back.');
  formData.append('submit_send_ticket_2', '1');

  // Maybe adding g-recaptcha-response as empty string?
  formData.append('g-recaptcha-response', '');

  const response = await fetch('https://ninegear.to/tickets/2-new-ticket?id_ticket=1846', {
    method: 'POST',
    headers: {
      'Cookie': cookie,
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Safari/537.36'
    },
    body: formData
  });

  const text = await response.text();
  fs.writeFileSync('post_response2.html', text);
  console.log('Saved to post_response2.html');
}

submit().catch(console.error);
